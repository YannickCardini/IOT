...
var TOPIC_LED = "sensors/led";
var TOPIC_PING = "sensors/ping";
...
const uri = "mongodb+srv://User2:miage@cluster0-h87a0.mongodb.net/Cluster0?retryWrites=true&w=majority";
...
client.connect(function(err,  mongodbClient){
	...
	app.get('/led/:etat', function (req, res) { //etat vaut on ou off
        etat = req.params.etat
		client_mqtt.publish(TOPIC_LED, etat);
        res.send(etat);
	});
	
	//PING vers un ESP spécifique
	app.get('/ping/:macAddress', function (req, res) { //etat vaut on ou off
		macAddress = req.params.macAddress;
		console.log(macAddress);
		client_mqtt.publish(TOPIC_PING, macAddress);
        res.sendStatus(200);
	});

	//PING vers un ESP spécifique
	app.get('/pingOff/:macAddress', function (req, res) { //etat vaut on ou off
		macAddress = req.params.macAddress;
		console.log(macAddress);
		client_mqtt.publish(TOPIC_PING, macAddress);
        res.sendStatus(200);
    ...
    client_mqtt.on('connect', function () {
		...
		client_mqtt.subscribe(TOPIC_PING, function (err) {
			if (!err) {
				//client_mqtt.publish(TOPIC_PING, 'Hello mqtt')
				console.log('Le serveur NODE a sub à sensors/ping ', TOPIC_PING);
			}
		})
    });
